	...
	<main class="p-t-50 m-t-10">
		<div class="container">
			<div class="d-flex justify-content-between">
				<h4 class="mb-0" style="font-weight: bold; color: black;">Q&A</h4>
			</div>
				...
				<tbody>
					<c:forEach items="${list}" var="board">
						<tr>
							<td>${board.bid}</td>
							<td>${board.id}</td>
							<td><c:forEach begin="1" end="${board.bindent}">[관리자 답변]</c:forEach>
								<a href="/q_content_view?bid=${board.bid}&bgroup=${board.bgroup}">${board.btitle}</a></td>
							<td>${board.bdate}</td>
							<td>${board.bhit}</td>
						</tr>
					</c:forEach>
					<tr>
						<sec:authorize access="isAuthenticated()">
							<td colspan="5"><a type="button"
								class="btn btn-dark float-right m-r-10" href="/q_write_view">글작성</a></td>
						</sec:authorize>
					</tr>
				</tbody>
			</table>
				...
				<nav aria-label="Page navigation example">
					<ul class="pagination justify-content-center">
						<c:if test="${pageMaker.pre}">
							<a class="page-link"
								href="qlist${pageMaker.makeQuery(pageMaker.startPage - 1)}">«</a>
						</c:if>

						<!-- 링크를 걸어준다 1-10페이지까지 페이지를 만들어주는것  -->
						<c:forEach var="idx" begin="${pageMaker.startPage}"
							end="${pageMaker.endPage}">
							<a class="page-link" href="qlist${pageMaker.makeQuery(idx)}">${idx}</a>
						</c:forEach>

						<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
							<a class="page-link"
								href="qlist${pageMaker.makeQuery(pageMaker.endPage +1)}"> »
							</a>
						</c:if>
					</ul>
				</nav>
